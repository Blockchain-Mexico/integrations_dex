...
import {CustomTD, TH, TR} from "../common/table";
import {themeBreakPoints} from "../../theme/commons";
import {AaveLoadingState, ATokenData} from "../../util /aave/types";
import {getKnownTokens, isWethToken} from "../../util /known_tokens";
import {ButtonVariant, TokenBalance} from "../../util /types";
    ...
    const tokensRows = () =>
        aTokensData.map((tokenD: ATokenData, index) => {
            ...
            if (ethAccount && borrowBalance && reserve && userAccountData) {
                ...
                const formattedBorrowedBalance = tokenAmountInUnits(borrowBalance, token.decimals, token.displayDecimals);
                const formattedAvailableForBorrowBalance = tokenAmountInUnits(availableBalanceForBorrowToken, token.decimals, token.displayDecimals);
    ...
    if (aaveLoadingState === AaveLoadingState.Loading || aaveLoadingState === AaveLoadingState.NotLoaded) {
        ...
        const handleSubmit = async (amount: BigNumber, token: Token, aToken: ATokenData, isEth: boolean) => {
            ...
            try {
                if (isBorrowState) {
                    // @ts-ignore
                    dispatch(amount, token, aToken, isEth, Protocol.Aave);
                } else {
                    // @ts-ignore
                    dispatch(amount, token, aToken, isEth, Protocol.Aave);
                }
